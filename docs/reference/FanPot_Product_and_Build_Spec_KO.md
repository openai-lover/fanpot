# FanPot — 제품 기획 및 구현 명세서

작성·공식 자료 확인 기준: **2026년 9월 21일, 한국 시간**. 대상: 1인 개발자의 Arc Microgrants 제출용 Mainnet MVP와 이후 제한적 상용 파일럿.

이 문서는 구현할 제품을 확정한 개발 명세다. 실제 웹사이트·GitHub 저장소·계약을 이번 기획 작업에서 생성하거나 배포했다는 뜻은 아니다. 아래의 주소·거래 증거는 개발 단계에서 실제로 생성해야 한다. 제품 정책은 **확정 설계**, 외부 사실은 연결된 **공식 출처**, 아직 확인할 수 없는 내용은 **미검증 또는 출시 전 확인 항목**으로 구분한다.
...
## 2. 2026년 최신 조사 결과와 제품에 반영할 사항

### 2.1 Arc Mainnet과 Microgrants

Arc는 **2026년 9월 16일 Public Mainnet 출시**를 공식 발표했다. USDC gas, EVM 호환성, deterministic sub-second finality를 제공한다. 개인정보 보호 기능은 전체 네트워크 출시를 위한 개발 중이라고 발표하므로, FanPot에서 비공개 거래 지원을 전제하지 않는다. [Arc Mainnet 출시 발표](https://www.arc.io/pressroom/circle-launches-arc-mainnet-an-economic-operating-system-for-the-internet)

Microgrants는 **실제 Arc Mainnet에서 작동하는 결과물, 공개 저장소, 열어볼 수 있는 live 링크, 제품·Arc 사용 설명, 공개 builder profile**을 요구한다. Testnet만 있는 결과물과 디자인 시안은 해당하지 않는다. 공지는 20건 × 500 USDC, 2026년 10월 14일 23:59 ET 마감, 10월 21일까지 결정으로 안내한다. 현재 날짜에서 환산한 마감은 **10월 15일 12:59 KST**다. 날짜 변경 가능성이 있으므로 제출 당일 재확인한다. 내부 제출 목표는 **10월 9일 KST**다. [Arc Microgrants 공식 공고](https://community.arc.io/public/events/arc-microgrants-f8tijfjhyq)

일반 Circle Developer Grants는 사업·마일스톤·성과를 더 중점적으로 보며 Microgrants와 절차가 다르다. 이번에는 작은 Mainnet 제품을 제출하고, 사업 확장은 후속 일반 grant의 근거로 쌓는다. [Circle Developer Grants](https://www.circle.com/grant)

### 2.2 Mainnet 설정 — 구현에서 그대로 사용할 기준값
...
공식 연결 문서는 wagmi/viem과 외부 지갑을 안내하며, native USDC와 ERC-20 USDC는 **같은 잔액**이므로 두 자산처럼 합산·표시하지 말라고 명시한다. [Connect to Arc](https://docs.arc.io/arc/references/connect-to-arc) · [공식 계약 주소](https://docs.arc.io/arc/references/contract-addresses)

FanPot 결정: `contribute(uint256 amount, bytes32 clientRef)`는 nonpayable이며 `SafeERC20.safeTransferFrom`을 사용한다. 금액은 전부 6자리 정수로 기록한다. gas 추정·잔액 여유 계산에만 18자리 native 단위를 사용한다. 예: 참여금 10 USDC = `10_000_000`, native 잔액 비교용 값 = `10_000_000 * 10^12`.

Arc 문서의 gas 안내는 `maxFeePerGas` 최소 20 Gwei를 설명한다. 그 페이지의 native transfer 예시에는 6자리 변환이 섞여 있어 그대로 복사하면 안 된다. **주소·native 모델 문서의 18/6 구분을 기준으로 삼고 실제 RPC와 소액 테스트로 검증한다.** gas 가격은 RPC 추정값과 최소값 중 큰 값으로 정하고, 고정 달러 수수료를 약속하지 않는다. [Gas and fees](https://docs.arc.io/arc/references/gas-and-fees) · [Stablecoin native model](https://docs.arc.io/arc/concepts/stablecoin-native-model)

**문서 충돌 처리:** 공식 `llms.txt`에는 아직 “Testnet only” 문구가 남아 있지만, 최신 출시 발표와 Mainnet 연결·계약 주소 페이지가 Mainnet을 명시한다. 따라서 최신 개별 문서와 배포 시 RPC 검증을 우선한다. USDC Transfer 이벤트 설명도 문서마다 차이가 있어, 자금 장부는 USDC 전체 Transfer 로그가 아닌 **FanPot 계약이 발생시킨 고유 이벤트**로 색인한다. [Arc 문서 인덱스](https://docs.arc.io/llms.txt) · [USDC 이벤트 색인 문서](https://docs.arc.io/integrate/infrastructure/indexing-events)

### 2.3 Arc를 쓰는 구체적인 이유

| Arc 특성 | FanPot의 실제 사용 | 심사에서 보여줄 증거 |
|---|---|---|
| native USDC + USDC gas | 참여·환불과 수수료에 별도 변동성 토큰이 필요 없음 | 지갑 잔액에 USDC 하나, 실제 contribution receipt |
| EVM 실행 | deadline·목표·고정 지급처·환불 규칙을 Solidity 계약에서 강제 | 실패 상태 payout revert 테스트와 배포 bytecode |
| deterministic finality | 정상 receipt 확인 후 짧은 대기와 참여 완료 표시 | 실제 receipt, supporter 목록 갱신 |
| 공개 settlement | 운영자의 엑셀과 별개로 모금·지급·환불을 대조 | 캠페인 주소와 ledger 합계 일치 |
| 글로벌 USDC 인프라 | 한국 개인 계좌 없이 이미 USDC를 가진 해외 팬 참여 | Arc 지갑 → 캠페인 계약 → 환불 주소 거래 |

Sub-second는 합의의 finality 설명이다. 지갑 승인, RPC, 서버 색인, 앱 복귀까지 모두 1초라는 SLA가 아니다. 정상적인 성공 receipt 한 번과 해당 블록 확인으로 성공 처리하며, 색인이 늦으면 “참여 완료 · 목록 반영 중”을 보여준다. [Deterministic finality](https://docs.arc.io/arc/concepts/deterministic-finality)

Arc만 사용할 수 있는 알고리즘이라고 주장하지 않는다. FanPot에서 중요한 것은 **USDC 참여금과 gas가 같은 자산이고, 정산 규칙과 그 실행 증거가 같은 체인에 있는 조합**이다. 한국 업체의 원화 수령과 해외 팬의 USDC 구매 문제는 별도의 현실적 단계로 남는다.

### 2.4 Wallet tooling·embedded wallet·AA·gas sponsorship 조사

| 기능 | 공식 자료에서 확인한 내용 | FanPot 결정 |
|---|---|---|
| 외부 지갑 | Arc 연결 문서가 MetaMask·Rabby 등과 wagmi/viem·WalletConnect를 안내 | MVP 채택. bare wagmi connectors + 자체 결제 UI |
| 이메일·소셜 | Circle User-controlled Wallets가 이메일 OTP와 Google·Apple·Facebook, PIN 방식을 안내 | Post-MVP: **이메일 OTP 기반 user-controlled SCA**를 첫 경로로 확정 |
| passkey | Circle Modular Wallets가 passkey와 MSCA를 지원 | 별도 wallet stack 증가를 피하기 위해 첫 확장에도 추가하지 않음 |
| Arc Mainnet 지원 | 현재 supported-blockchains 표에 ARC / ARC-TESTNET 및 EOA·SCA·MSCA 표시 | 지원 불가라고 단정하지 않음. 실제 계정·SDK 통합 테스트 필요 |
| account abstraction | SCA가 batch execution·gas sponsorship에 적합 | MVP는 EOA 경로. Post-MVP approve+contribute batch |
| Gas Station | 지원표에 Arc Mainnet user/developer-controlled 지원. 개발자가 gas 부담 | Post-MVP 이메일 SCA와 함께 도입 |
| Circle Paymaster | 다른 체인의 native gas 대신 USDC 결제를 위한 제품. 공개 제품 페이지의 체인 목록은 Arc 지원표와 동일하지 않음 | Arc 자체가 USDC gas이므로 MVP에 별도 Paymaster 불필요 |
| Circle App Kit·CCTP·Gateway | bridge·유동성 이동·체인 간 잔액 등의 도구 | MVP에 통합하지 않음. funding 도움말은 공식 지원 자료로 연결 |

근거: [Circle 지원 네트워크](https://developers.circle.com/wallets/supported-blockchains), [User-controlled Wallets](https://developers.circle.com/wallets/user-controlled), [Modular Wallets](https://developers.circle.com/wallets/modular), [Gas Station](https://developers.circle.com/wallets/gas-station), [Circle Paymaster](https://www.circle.com/paymaster), [Arc AA](https://docs.arc.io/arc/tools/account-abstraction), [App Kit FAQ](https://community.arc.io/public/blogs/app-kit-faqs-2026-07-07).

초기 지갑 연결은 기술의 부재 때문이 아니라 일정·복구·키 관리·지원 부담을 줄이기 위한 선택이다. embedded wallet만 넣어도 카드 구매나 원화 지급이 자동 해결되는 것은 아니다. MVP 참여자는 Arc USDC를 이미 보유했거나 별도 공식 경로로 준비할 수 있는 성인 팬으로 한정해 안내한다. 사용자의 시드나 개인키를 FanPot에 입력시키지 않는다.

### 2.5 팬덤 경제와 실제 경쟁 맥락

2026 해외한류실태조사는 30개국·지역의 한국 문화콘텐츠 경험자 27,400명을 대상으로 하며 조사 기간은 2025년 11~12월이다. **2026 발간 자료와 2026 실시간 소비 수치를 혼동하지 않는다.** 이는 해외 팬 시장의 국가별 맥락 자료이며 팬 광고 모금액 TAM을 직접 산출하는 자료는 아니다. [문체부·KOFICE 2026 조사 공식 아카이브](https://www.archivecenter.net/hallyuresearch/archive/srch/ArchiveNewSrchView.do?i_id=148724)

KOFICE의 2026 한류NOW는 생일 카페를 다루고, FanPlus는 생일·데뷔 기념일 투표와 팬 제작 광고를 운영한다. Mubeat 약관도 팬들의 모금을 통한 팬덤 광고를 정의한다. 따라서 FanPot은 새로운 팬 활동을 교육하는 서비스보다 **이미 존재하는 공동 프로젝트의 돈 관리 과정**에 집중해야 한다. [KOFICE 한류NOW 아카이브](https://www.archivecenter.net/hallyuresearch), [FanPlus 공식 FAQ](https://fanplus.co.kr/en-US/faq), [Mubeat 약관](https://api.mubeat.tv/view/terms/use?lang=KO)

| 대안 | 공식 자료로 확인한 강점·구조 | FanPot이 다르게 제공할 것 |
|---|---|---|
| 팬계정 계좌·PayPal 모금 | 빠르게 시작하고 기존 관계망 활용. 모든 운영자의 실태를 하나로 일반화할 공신력 있는 통계는 이번 조사에서 확보하지 못함 | 시작 전에 자금 규칙을 고정하고, 운영자 승인 없이도 실패 환불 청구 가능 |
| FanPlus | 전 세계 팬 커뮤니티, 생일·기념일 투표와 광고 보상. FAQ는 결제 없는 투표를 안내 | 투표권 경쟁이 아닌 특정 팬 프로젝트의 실제 예산 공동 조달 |
| Mubeat | 팬덤 광고·유료 서비스·플랫폼 환불 운영 | 자체 포인트 대신 실제 USDC 기여와 계약상 환불 권리 |
| MAKESTAR | 팬 프로젝트·이벤트와 결제·환불 체계. 최신 개인정보 처리방침에 결제 통화·수단과 환불 계좌 처리가 명시됨 | 독립 팬 운영자의 고정 지급처·예산 상한·미집행액 반환 규칙 |
| Kickstarter | 목표·deadline 고정과 all-or-nothing이 이미 존재 | “목표 실패 보호” 자체를 발명했다고 주장하지 않음. FanPot은 성공 후에도 항목별 지급 규칙과 잔액 반환이 이어짐 |
| Juicebox | 범용 programmable project funding/treasury 기반 | 범용 규칙·토큰화 설계 대신 K-pop 운영자가 이해할 단일 지급·환불 정책과 팬 UX |

근거: [MAKESTAR 2026 개인정보 처리방침](https://policies.makestar.com/privacy-policy/ko/20260212/), [MAKESTAR 펀딩 약관—2022 문서, 현행 전체 약관으로 간주하지 않음](https://policies.makestar.com/makestar-terms/en/20220802/), [Kickstarter 공식 all-or-nothing 설명](https://updates.kickstarter.com/why-is-funding-all-or-nothing/), [Juicebox V5 공식 문서](https://docs.juicebox.money/dev/v5/learn/overview/).

해외 팬의 한국 계좌 접근 어려움과 별도 해외 결제 창구 사용은 구매 지원업체의 운영 가이드에서도 관찰되지만, 이해관계가 있는 사업자의 설명이지 전체 팬덤의 통계는 아니다. **얼마나 자주 실패하는지, 총비용이 얼마나 줄어드는지는 10명 인터뷰와 파일럿의 실제 경로별 비용으로 검증한다.** [Ppasong 생일 서포트 구매 가이드—현장 보조 자료](https://www.ppasong.com/buy-guides/kpop-birthday-support-project-buying)

FanPot의 차별화 문장: **“누가 얼마나 모았는지 보여주는 데서 끝나지 않고, 정해진 곳에 정해진 한도만 지급하고, 남은 돈을 팬이 직접 돌려받게 한다.”** 경쟁사의 공개 자료에서 동일 기능을 확인하지 못한 것을 기능 부재나 FanPot의 세계 최초 주장으로 바꾸지 않는다.

## 3. Must / Nice / Post-MVP / Excluded

| 우선순위 | 구현 범위 |
|---|---|
| **Must Have** | 캠페인 생성·초안, 운영자 제한, 검토자 시작 승인, artist/project name·title·hero·description·goal·deadline·예산 1~3개, 고정 환불·잔액 정책, 상세·모금 진행률·고유 참여 지갑 수, USDC 승인 및 참여, 닉네임/X/익명과 금액 표시 선택, 즉시 receipt 확인, 성공·실패 판정, 항목별 지급 요청·검토·지급, 미집행 종료기한, 개별 환불 claim, 내 참여, 지출·환불·잔액·Arc 링크, 증빙 업로드, 최소 영·한 UI, 모바일·접근성·오류·보안 테스트, Mainnet 증거 패키지 |
| **Nice to Have** | 성공 시 1회 짧은 하트 애니메이션, 첫 참여 순번, Web Share API와 링크 복사. 지급·환불 검증이 끝난 뒤 합계 반나절 이내에서만 추가 |
| **Post-MVP** | 이메일 OTP embedded SCA + Gas Station, 승인·참여 batch, 적법한 fiat on/off-ramp 파트너, 원화 업체 정산, 2% 성공 집행 수수료, 더 큰 한도·외부 감사, email 알림 opt-in, 기본 CSV, 통제권 복구 설계, 영수증 진위 확인 프로세스 |
| **Do Not Build / Excluded** | 자체 토큰·NFT·staking·yield·DeFi, DAO 투표, chat/feed/팔로워 시스템, fan ranking·고액 기부자 우대, 아티스트 공식 인증처럼 보이는 배지, 크로스체인 결제·bridge 내장, 직접 환전·국제송금업, 리워드 배송·티켓 추첨·앨범 공동구매, Donate/Roll over, 오픈 무제한 캠페인 개설, 운영자 임의 출금·규칙 변경, 자동 loop 대량 환불, custom cryptography, AI 영수증 자동 승인, 멀티체인·모바일 네이티브 앱 |

“Follow”는 별도의 소셜 기능이 아니다. 내 참여에 프로젝트가 자동으로 나타나고, 상세의 공지·지급·환불 상태를 다시 확인하는 것으로 충족한다. 기능을 추가하기 전 두 질문을 적용한다: **팬이 더 쉽게 참여할 수 있는가? 자금이 사전 규칙대로만 움직이는가?** 둘 중 하나를 해치면 축소한다.
...
## 11. 수익모델과 3 organizers + 100 supporters 확보

### 단일 수익모델

**상용 계약 v2에서 실제 성공 집행금의 2% 서비스 수수료**를 받는다. 모금만 성공하고 전액 환불되면 수수료 0, 실패 환불 0, 미집행 잔액 0이다. 제출용 v1은 0%로 고정하며 나중에 기존 캠페인에서 수수료를 끌어갈 수 없다.

상용 예: 업체 순수령 2,800 USDC + 서비스 수수료 56 USDC + 미사용 준비금 144 USDC = goal 3,000 USDC. 팬에게 fee·공급자 비용·남은 금액 정책을 시작 전에 공개한다. gas·on/off-ramp·FX 비용은 2%에 포함되는 것처럼 숨기지 않고, 실제 견적에서 별도 구분한다. v2의 fee recipient/rate/계산법은 캠페인 시작 전 고정한다.

3개 × 100 USDC 파일럿의 최대 수수료 상당액은 6 USDC에 불과하다. 초기 파일럿은 매출보다 운영 시간·참여 마찰·환불 신뢰를 검증한다. 수익성은 더 큰 캠페인에서 확인하되 한도를 먼저 올리지 않는다.

### 초기 고객 확보 실행안 — 실제 발송은 이번 작업 범위 아님

| 시점 | 실행 | 목표와 산출물 |
|---|---|---|
| 개발 1주차 | X에서 10~12월 생일 프로젝트, 기존 예산·정산 공개가 있는 팬계정 30개를 수작업으로 목록화 | 한국 운영 15, 국제 fanbase 15. 공개 프로필·최근 프로젝트·해외 결제 창구·시기 기록 |
| 개발 1~2주차 | 그중 해외 팬 참여가 있는 12개 계정에 맞춤 접촉. “새 코인” 설명 대신 정산/환불 업무 감소 데모 30초 | 인터뷰 6건, 운영자 후보 3명. 무차별 DM·자동 크롤링 금지 |
| 제출 전 | 운영자 3명에게 자신의 다음 프로젝트 예산·지급처·환불 규칙을 샘플 폼으로 작성해 보게 함 | 첫 프로젝트 생성 완료율, 용어 오해, reviewer 확보 여부 |
| 법률·정산 경로 확인 후 | 각 운영자가 자기 채널에 소액 파일럿 링크·정산 원칙·USDC 준비 방법을 공지 | 운영자 A 40명, B 35명, C 25명. 중복 제외 총 100명 목표 |
| 파일럿 운영 | 20분 온라인 참여 안내, 지갑 키는 사용자만 보유. 1~5 USDC 소액 참여부터 | 결제 성공·도중 이탈·준비 시간 측정. 무료 지급 자금은 유기적 유료 참여와 구분 |
| 정산 이후 | 공개용 결과물·지급 증빙·환불 완료 비율 정리, 운영자 후기 확보 | 다음 프로젝트를 맡기려는 운영자 수와 정산에 든 시간 |

인터뷰 질문은 실제 경험에 집중한다: 최근 프로젝트 해외 참여자는 어떻게 냈는가, 실패·취소 환불에 몇 건·몇 시간이 걸렸는가, 이미 USDC를 쓸 수 있는 팬 비율은 어느 정도인가, 업체는 어떤 방식으로 돈을 받는가, 독립 검토자가 가능한가. 팬 결제 비용 개선은 `USDC 확보 비용 + bridge + approve/contribute gas + off-ramp/FX + 서비스료 + 환불 gas`의 총비용으로 비교한다.

100 supporters는 **동의한 실제 참여자 기준의 사업 목표**다. 화면·계약의 unique wallet count와 구분하며 다중 지갑을 사람 수로 뻥튀기하지 않는다. A/B/C 간 중복은 참여 후 자발적인 파일럿 확인으로 수작업 정리하고 추가 개인정보를 공개하지 않는다. 1인 개발자의 동시 운영 캠페인은 3개로 제한한다.

성과 기준: 운영자 3명 실제 발행, 중복 제외 실제 supporter 100명, 정상 참여 receipt 확인율 98% 이상, 환불 청구 성공률 100%(체인/토큰 제약 제외 사유별 기록), 수동 잔액 조작 0, indexer 누락 0, 운영자 최소 2명이 다음 프로젝트 사용 의사. 수치는 달성한 실적이 아니라 측정 목표다.

엔터테인먼트 회사 제휴·아티스트 계정 승인·공식 팬클럽 협약은 전제로 하지 않는다.
...
장기 정의: **“팬 커뮤니티가 공동 프로젝트 예산을 만들고, 지출 규칙과 잔액 반환을 실행하는 글로벌 팬 프로젝트 운영 도구.”**

K-pop → J-pop → VTubers → creators → sports fandom → global fandoms 순서는 사용 시장의 확장 후보이지 v1 roadmap의 추가 기능 목록이 아니다. 공통으로 artist/project·목표·deadline·공급자·예산·환불을 재사용할 수 있다. 다만 VTuber·creator의 수익배분, 스포츠 티켓·베팅, 투자성 상품은 기존 모델과 다른 법적·제품 범위이므로 자동 확장하지 않는다.

확장 조건은 현재 K-pop 파일럿에서 재사용 운영자 2명 이상, 성공·환불 운영 안정성, 적법한 정산 경로, 환불권을 해치지 않는 단위 수익성이다. 다국어 10종·멀티체인·팬 커뮤니티 SNS를 먼저 만들지 않는다.

## 14. 출처 사용 및 미확정 항목

모든 링크는 2026-09-21에 웹으로 확인했다. Arc의 출시 상태·네트워크 설정은 최신 공식 발표와 Mainnet 문서를 우선했다. 경쟁 서비스는 공식 약관·FAQ에서 확인한 범위만 비교했고, 특정 서비스의 기능 부재를 추정으로 단정하지 않았다. 법률 자료는 위험 식별의 근거이며 최종 영업 적법성 판단은 아니다.

다음은 개발자가 새 제품 결정을 내릴 항목이 아니라 **실제 배포 시 값·증거를 채울 운영 항목**이다.

- 개발자의 실제 GitHub URL·live domain·builder profile.
- Factory/reviewer/organizer/demo 지갑 주소와 배포 block/tx.
- Vercel·Supabase·WalletConnect 계정·비밀값, 실제 지갑 호환성 결과.
- Mainnet explorer의 source verification API 동작. 공식 배포 예제가 Testnet 중심이므로 Mainnet에서 검증 성공을 확인하기 전에는 “Verified” 표시 금지.
- 실제 광고 업체·원화 정산 파트너·허용 국가·법정 약관. 미확보 상태는 감추지 않음.
- 외부 코드 감사 여부. 테스트 통과와 소스 공개를 audit 완료로 표현하지 않음.
