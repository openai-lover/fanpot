...
export function MainnetLaunch() {
  ...
  return <div className="launch-grid">
    ...
    <section className="mainnet-panel"><h2>4. Reviewer activation</h2><p>On the other computer, connect {reviewer} and confirm activation. The reviewer already has 5 USDC for Arc gas.</p><button className="button" disabled={busy || !isAddress(campaign)} onClick={activate}>Activate with reviewer wallet</button>{isAddress(campaign) && <a href={`/launch?campaign=${getAddress(campaign)}`}>Reviewer link ↗</a>}</section>
