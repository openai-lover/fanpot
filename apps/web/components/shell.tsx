...
const demoProjects = [
  { id: 'ad-campaign', image: '/arc-demo/subway-display-idol-v2.jpg', status: 'TESTNET CAMPAIGN', title: 'LUMI birthday screen', description: 'Proposed subway display and original artwork, with a live testnet funding balance.', alt: 'AI-generated fictional adult performer LUMI on a proposed subway screen ad' },
  { id: 'goods-campaign', image: '/arc-demo/goods-mockup-idol-v2.jpg', status: 'SETTLED · TESTNET', title: 'LUMI fan-made goods', description: 'Cupsleeve and fictional artist photocard concept with simulated payout and refund receipts.', alt: 'AI-generated fictional LUMI portrait photocards and cupsleeve concept' },
  { id: 'cafe-campaign', image: '/arc-demo/cafe-concept-idol-v2.jpg', status: 'UNUSED FUNDS RETURNED', title: 'LUMI birthday café', description: 'A full fund with an optional expense skipped and remaining funds returned.', alt: 'AI-generated fictional LUMI portrait in a birthday café concept' },
  { id: 'cancelled-campaign', image: '/arc-demo/bus-shelter-idol-v2.jpg', status: 'CANCELLED · REFUNDED', title: 'LUMI bus shelter proposal', description: 'A stopped campaign showing that supporters recover all testnet contributions.', alt: 'AI-generated fictional LUMI portrait in an unbooked bus shelter ad concept' },
];
export function Home() {
  const { messages: m } = useContext(I18n);
  return <main id="main" tabIndex={-1}><section className="hero"><div className="hero-copy"><span className="eyebrow"><span className="tiny-star">✦</span>{m.eyebrow}</span><h1>{m.tagline}</h1><p>{m.intro}</p><div className="hero-actions"><a className="button" href="#projects">{m.browse}<ArrowUpRight size={18}/></a><a className="text-link" href="#rules">{m.how}<span>↗</span></a></div><div className="hero-note"><span className="heart-badge"><Heart size={15}/></span>Fictional AI artist LUMI · Arc Testnet concept</div></div><div className="hero-art"><div className="art-label"><span/>FICTIONAL AI ARTIST</div><div className="hero-mockup"><Image src="/arc-demo/subway-display-idol-v2.jpg" width={1536} height={1024} alt="AI-generated fictional adult performer LUMI on a proposed subway digital screen; the ad has not been installed" priority sizes="(max-width: 600px) 88vw, 48vw" /></div><span className="art-sticker"><Heart size={17} fill="currentColor"/>Imagined together</span><span className="floating-spark">✦</span></div></section>
    <section className="projects-section" id="projects"><div className="section-heading"><div><span className="eyebrow">OUR NEXT CHAPTER</span><h2>{m.projectHeading}</h2><p>Explore what each fictional campaign proposes to make, alongside its Arc Testnet fund flow. LUMI is an AI-generated fictional performer.</p></div><span className="small-pill">VISUAL CONCEPTS · ARC TESTNET</span></div><div className="campaign-teasers">{demoProjects.map((project) => <Link className="campaign-teaser" href={`/arc-demo#${project.id}`} key={project.id}><div className="campaign-teaser-image"><Image src={project.image} width={1536} height={1024} alt={project.alt} sizes="(max-width: 750px) 100vw, 50vw"/><span>AI-GENERATED CONCEPT</span></div><div className="campaign-teaser-copy"><span className="small-pill">{project.status}</span><h3>{project.title}</h3><p>{project.description}</p><strong>See campaign <ArrowUpRight size={16}/></strong></div></Link>)}</div></section>
    <RulesCard/>
