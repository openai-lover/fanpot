...
const deployment: { factory: string | null; campaign: string | null; vendor: string | null; organizer: string; reviewer: string } = rawDeployment;
...
async function readCampaign() {
  ...
  try {
    ...
    if (chainId !== 5042 || !factoryCode || !campaignCode || !registered || config.usdc.toLowerCase() !== ARC_USDC.toLowerCase() || config.organizer.toLowerCase() !== deployment.organizer.toLowerCase() || config.reviewer.toLowerCase() !== deployment.reviewer.toLowerCase() || allocation.recipient.toLowerCase() !== deployment.vendor.toLowerCase()) return null;
