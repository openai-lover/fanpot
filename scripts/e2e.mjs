...
const port = Number(process.env.FANPOT_E2E_PORT || '3000');
if (!Number.isInteger(port) || port < 1024 || port > 65535) throw Error('Invalid FANPOT_E2E_PORT');
const origin = `http://127.0.0.1:${port}`;
try { await fetch(origin, { signal: AbortSignal.timeout(1000) }); throw Error(`Port ${port} already in use; stop the preview before E2E.`); }
catch (e) { if (e.message.startsWith(`Port ${port}`)) throw e; }
const server = spawn(process.execPath, [webRequire.resolve('next/dist/bin/next'), 'start', '--hostname', '127.0.0.1', '--port', String(port)], { cwd: resolve('apps/web'), stdio: 'ignore' });
...
try {
  ...
  for (let i = 0; i < 100 && server.exitCode === null; i++) {
    try { const r = await fetch(`${origin}/api/health`, { signal: AbortSignal.timeout(1000) }); if (r.ok) { ready = true; break; } } catch { /* server still starting */ }
